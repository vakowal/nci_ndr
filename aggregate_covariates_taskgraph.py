...
def zonal_sum_to_csv(input_raster_path, sum_field_name, save_as):
    ...
    zonal_df.rename(columns={'sum': sum_field_name})
    zonal_df.drop(['min', 'max', 'count', 'nodata_count'], axis='columns')
...
def aggregate_proportion_irrigation(area_df_path, save_as):
    ...
    irrigated_area_df.rename(columns={'sum': 'irrigated_area_sum'})
    total_area_df = pandas.read_csv(area_df_path)
    irrigated_area_df.merge(
    ...
    irrigated_area_df.drop([
        ...
        'basin_sum_area_km2'], axis='columns')
    irrigated_area_df.to_csv(save_as)
...
def reclassify_urban_extent(save_as):
    ...
    pygeoprocessing.reclassify_raster(
        _COVARIATE_PATH_DICT['urban_extent'], value_map, save_as,
...
def aggregate_proportion_urban(urban_extent_rc_path, save_as):
    ...
    urban_extent_df.drop(
        ['min', 'max', 'count', 'nodata_count', 'sum'], axis='columns')
    urban_extent_df.to_csv(save_as)
...
def reclassify_countries_by_sanitation(save_as):
    ...
    pygeoprocessing.reclassify_raster(
        ...
        countryid_to_sanitation, save_as, gdal.GDF_Float32, -9999.)
...
def aggregate_covariates(intermediate_dir_path, combined_covariate_table_path):
    ...
    # basin_area_task = graph.add_task(
    #     zonal_sum_to_csv,
    #     args=(
...
def main():
    """Program entry point."""
    intermediate_dir_path = 'C:/Users/ginge/Desktop/intermediate_df_dir'
    combined_covariate_table_path = 'C:/Users/ginge/Desktop/combined_covariates.csv'
